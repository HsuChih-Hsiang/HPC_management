...
def load_notified_emails_history():
    """載入每年已通知信箱的歷史紀錄。"""
    if not os.path.exists(params.HPC_NOTIFIED_EMAILS_YEARLY_FILE):
        return {}
    try:
        with open(params.HPC_NOTIFIED_EMAILS_YEARLY_FILE, 'r', encoding='utf-8') as f:
            history = json.load(f)
            if not isinstance(history, dict):
                raise ValueError("hpc_notified_emails_yearly.json is not a dictionary")
            return history
    except (json.JSONDecodeError, ValueError):
        print(f"警告: {params.HPC_NOTIFIED_EMAILS_YEARLY_FILE} 檔案損壞或格式不正確，將重新創建。")
        return {}

def save_notified_emails_history_log(history):
    """儲存每年已通知信箱的歷史紀錄。"""
    with open(params.HPC_NOTIFIED_EMAILS_YEARLY_FILE, 'w', encoding='utf-8') as f:
...
def save_hpc_notification_log(notification_record):
    ...
    """
    # 確保根目錄存在
    os.makedirs(params.HPC_NOTIFICATIONS_DIR, exist_ok=True)
    ...
    # 根據時間戳建立分層的資料夾路徑
    year_dir = os.path.join(params.HPC_NOTIFICATIONS_DIR, record_time.strftime('%Y'))
